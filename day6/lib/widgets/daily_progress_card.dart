    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(
          ...
          colors: [Color(0xFFFFF2EA), Color(0xFFFFE6D9)],
        ),
...
class GradientProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Light orange at TOP -> Dark orange at BOTTOM
    // ------------------------------------------------------------------------

    const double startAngle = -math.pi / 2;

    // The progress starts exactly at 12 o'clock.
    final double sweepAngle = 2 * math.pi * progress;

    // Colors
    const Color lightOrange = Color(0xFFFFB27D);
    const Color darkOrange = Color(0xFFFF6252);

    // ------------------------------------------------------------
    // Draw the progress using small segments.
    // The color is based on the POSITION around the circle,
    // NOT based on how much progress has been completed.
    // ------------------------------------------------------------

    const int totalSegments = 120;

    final int activeSegments = (totalSegments * progress)
        .clamp(1, totalSegments)
        .toInt();
    ...
    for (int i = 0; i < activeSegments; i++) {
      ...
      // ----------------------------------------------------------
      // COLOR POSITION AROUND THE CIRCLE
      //
      // 0.00 = TOP       -> LIGHT
      // 0.25 = RIGHT     -> BLEND
      // 0.50 = BOTTOM    -> DARK
      ...
      final double position = t1;

      late Color segmentColor;

      if (position <= 0.25) {
        // TOP -> RIGHT
        // Light orange gradually becomes dark orange
        final double ratio = position / 0.25;

        segmentColor = Color.lerp(lightOrange, darkOrange, ratio)!;
      } else if (position <= 0.75) {
        // RIGHT -> LEFT through the BOTTOM
        // Dark orange remains dominant
        segmentColor = darkOrange;
      } else {
        // LEFT -> TOP
        // Dark orange gradually becomes LIGHT orange
        final double ratio = (position - 0.75) / 0.25;

        segmentColor = Color.lerp(darkOrange, lightOrange, ratio)!;
      }
